...
from app import app
from .db import insertCategory, getCategories, getCategoryForId, updateCategory, insertItem, getItems, getItemForId, updateItem, getDeletableCategories, deleteCategory, insertBarcode, getBarcodesForItem, getBarcodes, deleteBarcode, getItemsForCategory, deleteItem, getBarcode, scanBarcodeAndUpdateQuantity, getUsers, updateUserRole, searchAndUpdateQuantity, insertHistory, getHistory
...
def returnPermissionError():
    insertHistory("PERMISSION_ERROR", current_user, "Attempted to access an unauthorized resource.")
...
@app.route('/')
def index():
    insertHistory("PAGE_VISIT", current_user, "Viewed index.")
...
def login_callback():
    ...
    if userinfo_response.json().get("email_verified"):
        ...
        login_user(user)
        insertHistory("LOGIN", user, "Logged in.")
    else:
...
@login_required
def logout():
    insertHistory("LOGOUT", current_user, "Logged out.")
    logout_user()
    return redirect('/dashboard')

@app.route('/dashboard')
def dashboard():
    insertHistory("PAGE_VISIT", current_user, "Viewed dashboard.")
    return render_template('dashboard.html', USER=current_user)

@app.route('/feedback')
def feedback():
    insertHistory("PAGE_VISIT", current_user, "Viewed feedback.")
    return render_template('feedback.html', USER=current_user)

@app.route('/register/<string:type>', methods=['GET', 'POST'])
@login_required
def register_with_param(type):
    insertHistory("PAGE_VISIT", current_user, "Viewed register.")
    ...
    if type == 'category':
        ...
        if request.method == 'POST' and form_category.category.data and form_category.validate():
            ...
            insertHistory("REGISTER", current_user, "Registered category: " + str(form_category.category.data))
            return redirect('/dashboard')
        ...
        if request.method == 'POST' and form_item.category.data and form_item.item.data and form_item.validate():
            ...
            insertHistory("REGISTER", current_user, "Registered item. Category: " + str(form_item.category.data).strip() + ", Item: " + str(form_item.item.data).strip() + ", Location: " + str(form_item.location.data).strip())
            return redirect('/dashboard')
        ...
        if request.method == 'POST' and form_barcode.validate():
            ...
            insertHistory("REGISTER", current_user, "Registered barcode. Item: " + str(form_barcode.item.data).strip() + ", Barcode: " + str(form_barcode.barcode.data).strip())
            return redirect('/dashboard')
...
def register_barcode_for_item(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed register barcode.")
    if not validate_user():
        return returnPermissionError()
...
def edit_category(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed edit category.")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_category.category.data and form_category.validate():
        ...
        insertHistory("EDIT", current_user, "Updated category. UUID: " + str(uuid).strip() + ", Category: " + str(form_category.category.data).strip())
...
def delete_barcode_view():
    insertHistory("PAGE_VISIT", current_user, "Viewed delete barcode view.")
    if not validate_user():
        return returnPermissionError()
...
def delete_barcode(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed delete barcode.")
    if not validate_user():
        return returnPermissionError()
    deleteBarcode(uuid)
    insertHistory("DELETE", current_user, "Deleted barcode. UUID: " + str(uuid))
    return redirect('/delete/barcode')

@app.route('/delete/item/<string:uuid>')
@login_required
def delete_item(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed delete item.")
    if not validate_admin():
        return returnPermissionError()
    deleteItem(uuid)
    insertHistory("DELETE", current_user, "Deleted item. UUID: " + str(uuid))
    return redirect('/view')

@app.route('/delete/category')
@login_required
def delete_category_view():
    insertHistory("PAGE_VISIT", current_user, "Viewed delete category view.")
    if not validate_admin():
        return returnPermissionError()
...
def delete_category(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed delete category.")
    if not validate_admin():
        return returnPermissionError()
    deleteCategory(uuid)
    insertHistory("DELETE", current_user, "Deleted category. UUID: " + str(uuid))
    return redirect('/delete/category')

@app.route('/edit/item/<string:uuid>', methods=['GET', 'POST'])
@login_required
def edit_item(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed edit item.")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_item.validate():
        ...
        insertHistory("EDIT", current_user, "Edited item. UUID: " + str(uuid) + ", Category: " + str(form_item.category.data) + ", Location: " + str(form_item.location.data) + ", Quantity_Active: " + str(form_item.quantity_active.data) + ", Quantity Expired: " + str(form_item.quantity_expired.data) + ", Notes: " + str(form_item.notes.data) + ", URL: " + str(form_item.url.data))
        return redirect('/view/item/' + uuid)
    else:
...
def view_category(uuid):
    insertHistory("PAGE_VISIT", current_user, "Viewed category " + str(uuid))
...
@app.route('/view/all')
def view_all_items():
    insertHistory("PAGE_VISIT", current_user, "Viewed all items page")
...
def edit_user_role(user_id, new_role):
    insertHistory("PAGE_VISIT", current_user, "Viewed edit user API")
    if not validate_admin():
        return returnPermissionError()
    print("Updating for:")
    ...
    insertHistory("EDIT", current_user, "Edited user. UserId: " + str(user_id) + ", Role: " + str(new_role))
    return ""

@app.route('/barcode/check_in', methods=['GET', 'POST'])
@login_required
def barcode_check_in_item():
    insertHistory("PAGE_VISIT", current_user, "Viewed barcode checkin")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_barcode.validate():
        ...
        if scanBarcodeAndUpdateQuantity(form_barcode.barcode.data, form_barcode.quantity.data):
            ...
            insertHistory("EDIT", current_user, "Check in with barcode. Barcode: " + str(form_barcode.barcode.data) + ", Quantity: " + str(form_barcode.quantity.data))
...
def barcode_check_out_item():
    insertHistory("PAGE_VISIT", current_user, "Viewed barcode checkout")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_barcode.validate():
        if scanBarcodeAndUpdateQuantity(form_barcode.barcode.data, form_barcode.quantity.data):
            item_id = getBarcode(form_barcode.barcode.data)['item_id']
            insertHistory("EDIT", current_user, "Check out with barcode. Barcode: " + str(form_barcode.barcode.data) + ", Quantity: " + str(form_barcode.quantity.data))
            return redirect('/view/item/' + item_id)
        else:
            flash("An error occurred when handling your request. Please validate the barcode and quantity.")
...
def search_check_out_item():
    insertHistory("PAGE_VISIT", current_user, "Viewed search checkout")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_search.validate():
        if searchAndUpdateQuantity(form_search.selectInput.data, form_search.quantity.data):
            insertHistory("EDIT", current_user, "Check out with search. ItemId: " + str(form_search.selectInput.data) + ", Quantity: " + str(form_search.quantity.data))
...
def search_check_in_item():
    insertHistory("PAGE_VISIT", current_user, "Viewed search checkin")
    if not validate_user():
        return returnPermissionError()
    ...
    if request.method == 'POST' and form_search.validate():
        if searchAndUpdateQuantity(form_search.selectInput.data, form_search.quantity.data):
            insertHistory("EDIT", current_user, "Check in with search. ItemId: " + str(form_search.selectInput.data) + ", Quantity: " + str(form_search.quantity.data))
            return redirect('/view/item/' + form_search.selectInput.data)
        else:
            flash("An error occurred when handling your request. Please validate the selection and quantity.")
...
def barcode_look_up_item():
    insertHistory("PAGE_VISIT", current_user, "Viewed barcode lookup")
    form_barcode = Barcode_Lookup(request.form)
    if request.method == 'POST' and form_barcode.validate():
        item_id = getBarcode(form_barcode.barcode.data)['item_id']
        insertHistory("EDIT", current_user, "Barcode lookup. Barcode: " + str(form_barcode.barcode.data))
        return redirect('/view/item/' + item_id)
...
def view_history():
    insertHistory("PAGE_VISIT", current_user, "Viewed history lookup")
    if not validate_admin():
        return returnPermissionError()

    return render_template('history.html', USER=current_user, events=getHistory())

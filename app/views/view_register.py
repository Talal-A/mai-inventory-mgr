...
def register_with_param(type):
    ...
    form_subcategory = Register_Subcategory(request.form)
    ...
    if type == 'category':
        ...
        if request.method == 'POST' and form_subcategory.subcategory.data and form_subcategory.validate():
            database.insert_subcategory(form_subcategory.subcategory.data, form_subcategory.category.data)
            return redirect('/dashboard')

        return render_template('register:' + type + '.html', USER=current_user, form=form_subcategory)
        ...
        if request.method == 'POST' and form_item.category.data and form_item.name.data and form_item.validate():
            ...
            if not image_data:
                flash("A photo is required to register a new item.")
